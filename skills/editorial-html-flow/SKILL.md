...
用法：文字右键 →「编辑」→「完成」（Esc 取消，Enter 提交；编辑时可选颜色、改字号，也可点旁边的 token 色块换成主题色；自定义文本还可插入整行弱分割线）；标题右键「删除本组」会删掉标题及其下组件；空白处右键仍出编辑菜单，点「新增组件」才打开右侧组件面板，再点卡片插入目录里的父组件（自定义文本在导语下；含折线图、排行榜、表格、漏斗图、关系网络、节点表）。章头 / 小节标题 / 导语 / 来源 / 分割线 / 自定义文本不配图。顶栏最右侧「演示 / 编辑」切换：演示态关闭编辑，只留阅读与看图。组件上右键「复制组件」会在原块后插入一份（含当前文案与变体）。排行榜 / 表格 / 节点表 / 折线图 / 漏斗图 / 关系网络在组件上右键可新增或删除行和列（折线是系列与数据点，漏斗是节点，网络是起点与终点）。数字信息 / 编号信息 / 无编号信息 / 观点 / 议题格 / 自定义文本可拖高：同一行里各条顶对齐，整组按最高那条的高度在拉高后的容器里垂直居中。悬停左侧六点手柄拖动排序（多条目组件同时出「组件」「本条」两个手柄：组件拖整组，可换章节；本条只在本组内换顺序。拖动时保留内容预览，蓝线跟着手停的空挡；拖到底部垃圾桶可删除；组件排序时封面不显示）；编号信息等条目可右键新增/删除一条。拖章头 / 复制 / 删除章头时，紧跟的强线跟着走。独立分割线可靠近线条右键「删除组件」。图片右键可勾选「点击轮播」（动效背景下）：右侧出缩略图 + 图1 / +，没图时缩略图只留灰底；点 + 可一次多选加入，点图名替换该张，拖行可改轮播顺序；勾选后点击图片按顺序渐隐循环，不再打开大图。图片 / 视频 / SVG 图示右键 →「替换」或「尺寸」。替换可多选：未开轮播会自动打开并加入所选图片，已开则追加进现有轮播。写入 `lib/media/` 时按章节+节点单独命名，不要盖掉其他组件已有的文件。封面右键切版式（左齐 / 居中 / 配图 / 图文分栏），配图封面可替换图片；图文分栏封面高度固定为一屏。配图封面可勾选「动效背景」，右侧面板切换 Ribbon Field / Halftone Flow / Matrix Field / Stream Convergence 并调速度、色相、饱和等参数。编号信息右键改布局（格子 A / 格子 B / 列表 A / 列表 B），勾选「显示编号」后编号位置 / 类型 / 字号出现在菜单右侧，勾选「背景」后可设背景色，格子列数用 − N +（2–5）。编号信息和议题格右键可勾选「配图」；图在整组信息的左 / 右，配图面板在编号位置右侧，不要给每一条单独配图。配图右键可换裁切 / 可滚动 / 宽图 / 海报；图和说明在配图区域内上下左右居中，拖配图高度时整组与左侧信息跟着变高。数字信息右键勾选前缀 / 后缀 / 说明，列数用 − N +（2–5）；横排数字是同一父组件的紧凑排。无编号信息和议题格可切格子 A / B，议题格列数 1–5。观点可切换色块 / 浅底；图可切换裁切 / 可滚动 / 宽图 / 海报，右键可新增一条并用 − N + 改列数（1–5）。图墙拖高后各行均分，铺满只裁切且不超出容器。图表改数字后曲线跟着动。⌘Z / Ctrl+Z 撤销，⌘⇧Z / Ctrl+Y 重做（含换图、删组、新增、排序）。尺寸可改对齐（左/中/右）、容器宽度（拖右边手柄）、高度（拖下边手柄）、填充（适应 fit / 铺满 fill）。表和折线图同样可拖宽高。图表改数字后纵轴刻度按新最大值重算。基础 token 右键改数值，立刻作用于整页并写入当前主题；颜色 token 点色块用取色器，也可改 hex。主题导入导出在目录页。SVG 上的字可编辑，点空白图形则整图替换。
...
- **每个主标题**（章头 `.slide-head` / 判断句 h2）下方必须紧跟一条可见的**强线** `.rule`。不得省略，不得用「下一块顶线」替代，CSS 不得把这条线 `display:none`。缺线时补 markup；没有 `.rule` 时才允许用标题 `::after` 兜底，二者不要叠成双线。章头和强线仍是两个相邻节点，不要把 `.rule` 写进 `.slide-head`；编辑器排序 / 复制 / 删除章头时会带上紧跟的那条强线。独立插入的分割线可靠近线条右键删除。
