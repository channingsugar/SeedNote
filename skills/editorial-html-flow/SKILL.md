...
| 一句判断 | `.point`。默认左标签右正文、正文左对齐；浅底 `.is-soft` 单列左对齐，不要把正文推进第 2 列 |
...
| 表 | `.plain-table-wrap`。浅底表头、横竖弱线、外框左右底边、左列加粗。两行表头用 `.table-level` / `.table-score`；强调列 `th.is-accent` |
| 公式 | `.market-formula` |
...
HTML 里的路径按实际相对位置改。章结构：`.report-slide` 一章一块，块内 `.flow-stack` 上下叠。每章 `.slide-head` 里必须有 `.sub` 导语（写在分割线上方），下必须有 `.rule`。添加章头不要只插标题和线，也不要把章头导语写成线下面的 `.research-lead`。导航 `.toc` 要带 `data-component-id="navigation"`，链到各章 `id`。顶栏 tab 文案取每一个章头「题域」`.kicker`（封面固定「封面」），同一章里多枚章头各出一枚 tab；没有题域的空章不要出 tab，改题域即改 tab。点 tab 滚到该枚章头（不要滚到整章 `section`），即时改 `scrollTop`，短锁高亮，末章底部垫高。视频不要 `data-lightbox-src`、不要点开大图；`controls` 一直在，播放条用 CSS 默认隐藏、悬停才出，进度条可点可拖。
...
1. 从 `templates/editorial-flow/editor/` 把 `seed-edit.css`、`seed-edit.js`、`serve.py` 拷到报告旁，建议 `lib/editor/`（旧产出也可能是根目录 `editor/`）。从 `templates/editorial-flow/assets/` 把 `cover-fx.js` 拷到报告 `lib/`。把 `open-report.command` 拷到报告根目录并改名为 `打开报告.command`。
...
5. 用报告文件夹里的 `打开报告.command` 打开后再编辑。改动会写回磁盘上的 `index.html`（换图写入 `lib/media/`）。HTTP 打开时以磁盘 HTML 为唯一真相：不要用 IndexedDB 的 markup / layout 套回模块（会改高度、改结构、清空没碰过的块）。打开页面不要自动 persist。把**整个文件夹**发给别人，对方同样用「打开报告」即可看到结果并继续改。直接双击 HTML（`file://`）时修改只留在浏览器里，文件夹不会更新。目录页带 `data-catalog`，不读写 IndexedDB，也不写磁盘。
6. 主题导入导出只在目录 `templates/editorial-flow/index.html`。报告壳 `report.html` 不要做主题栏。

编辑器用法（`templates/editorial-flow/editor/`，细节以 `editor/README.md` 为准，改功能先改那里再同步本段）：

- **演示 / 编辑**：顶栏最右侧切换。演示态（`html.is-seed-presenting`）关闭右键、排序、新增、就地改字，只留阅读与看图。顶栏不要「新增组件」按钮。
- **新增组件**：空白处右键仍出原来的编辑菜单；只有点菜单里的「新增组件」才打开**右侧悬浮卡片面板**，再点卡片插入。插入「章头」时必须带上 `.sub` 导语（标题下、分割线上方）和强线。自定义文本排在独立导语块下。章头 / 小节标题 / 导语 / 来源 / 分割线 / 自定义文本**不配图**，库卡片纯文本、不要配图预览。
- **点击轮播**：图片右键、动效背景下勾选「点击轮播」（`data-cycle`）。右侧出缩略图 + 图1 / +；没图时缩略图只留灰底，不要斜杠。点 + 可一次多选加入；点图名替换该张；拖行改顺序。勾选后点击按顺序**渐现**切换，**不要打开大图**。
- **替换**：单张仍换当前图。多张且未开轮播 → 自动打开轮播并加入所选图；已开则全部追加。写入 `lib/media/` 时按你选中的原文件落盘，章节 + 节点单独命名（`data-media-id`），不要拿缓存或相邻槽位顶替，也不要盖掉别的组件已有文件。
- 文字右键 →「编辑」→ 点「完成」才提交（Esc 取消；回车只换行）。粘贴只进纯文本。可选颜色、改字号，点旁边 token 色块换主题色；自定义文本还可插入整行弱分割线。标题右键「删除标题」只删标题和紧跟的强线，不删后面内容。顶栏 tab 跟章头题域走；点 tab 滚到该枚章头，不要滚到整章。
- 组件右键「复制组件」在原块后插一份。排行榜 / 表格 / 节点表 / 折线图 / 漏斗图 / 关系网络可增删行和列。
- 排序：悬停左侧六点手柄。多条目同时出「组件」「本条」：组件拖整组、可换章；**本条只在本组内换序，不能拖出组件**。蓝线跟着手停的空挡。拖到底部垃圾桶删除。拖 / 复制章头时带上紧跟的强线（`.sub` 导语在章头里一起走）。删除标题只删标题（含 `.sub`）和强线。独立 `.rule` 可靠近线条右键删除。组件排序时封面不显示。
- 封面右键切左齐 / 居中 / 配图 / 图文分栏。配图和动效背景互不绑定；左齐 / 居中 / 分栏不要格子。「动效背景」只给封面。章头可勾选「背景」（颜色 / 配图 / 动效），有背景时不显示分割线。图片 / 视频容器（含封面）右键可「移除当前素材」。编号信息 / 议题格整组可配图（左 / 右），图和说明在配图区内居中。其余变体、尺寸、撤销重做见 `editor/README.md`。
...
- 顶栏只放「演示 / 编辑」，不要再加「新增组件」。空白右键必须仍是编辑菜单，只有菜单里的「新增组件」打开右侧卡片库。
- 章头 / 小节标题 / 导语 / 来源 / 分割线 / 自定义文本不要配图槽，库卡片不要配图预览。
- 勾了「点击轮播」的图不要走 lightbox；切换只渐现，不要闪铺满。
- 「本条」排序不能拖出原组件；落点蓝条必须跟当前空挡。
- 换图按选中的原文件写入，轮播每张图单独 `data-media-id` / 文件名，禁止用路径末尾截断或共享 IndexedDB 键盖掉别的图。
- HTTP 报告以磁盘 HTML 为真相：禁止用 IndexedDB 的 markup / 尺寸回写到没改过的模块；打开页面禁止自动 persist。
- hydrate 不要拆 `.point` 里非 `span` / `p` 的正文（自定义字号常包在 `div` 里）。
- `.point` 默认两列（标签 | 正文左对齐）；`.is-soft` 必须单列左对齐，禁止 `grid-column: 2` 造出隐式右列。
- persist 去掉运行时标记（`is-seed-hover-*`、`data-seed-video-hover`），不要靠这些属性跳过视频绑定。
- 「动效背景」只出现在封面，不要出现在章头、图片或其他不支持的组件。
- 封面配图与动效背景解耦；左齐 / 居中 / 图文分栏不要网格底纹。
- 章头「背景」可切颜色 / 配图 / 动效；有背景时隐藏紧跟的强线，但 `.sub` 导语必须还在色块里。
- 添加章头必须带上 `.sub` 导语（分割线上方）和强线，不要只插标题和线，也不要把这句导语放到线下面。
- 图片 / 视频容器（含封面）菜单要有「移除当前素材」；轮播只移除当前张。
...
- **每个主标题**（章头 `.slide-head` / 判断句 h2）里必须有 `.sub` 导语，写在分割线**上方**。下方必须紧跟一条可见的**强线** `.rule`。添加章头必须带上 `.sub` 和强线。不得省略强线，不得用「下一块顶线」替代，CSS 不得把这条线 `display:none`（章头勾选背景时除外，且不得藏 `.sub`）。缺线时补 markup；没有 `.rule` 时才允许用标题 `::after` 兜底，二者不要叠成双线。`.rule` 不要写进 `.slide-head`；`.sub` 必须写在章头里。编辑器排序 / 复制章头时带上紧跟的强线。删除标题只删标题（含 `.sub`）和强线。独立插入的分割线可靠近线条右键删除。独立导语块 `.research-lead` 仍是单独组件，不要当成章头里的那句导语。
...
| 章头 | `.slide-head` | 必须带 `.sub` 导语（分割线上方）和强线；`data-head-bg` 颜色 / 配图 / 动效，有背景时不显示紧跟强线，不藏 `.sub` |
