...
两套 Cursor Skill，对应两套不能互切的 HTML 模板。没说分页时默认走流式。

| | 流式 | 分页 / 分屏 |
|---|---|---|
| Skill | `editorial-html-flow` | `editorial-html-page` |
| 定义 | `.cursor/skills/editorial-html-flow/SKILL.md` | `.cursor/skills/editorial-html-page/SKILL.md` |
...
两套各自维护 token 和编辑器，不要交叉引用，也不要加「流式 / 分屏」切换。

## 怎么用

在 Cursor 对话里直接说意图即可。Agent 会按 description 匹配 Skill，不必先 `@` 文件。说得越明确，越不会走错套。

也可以点名：

```
按 editorial-html-flow 把这份大纲做成流式报告
按 editorial-html-page 只组织章节树，先不要出 HTML
```
...
- 流式：http://127.0.0.1:8766/templates/editorial-flow/index.html
- 分页：http://127.0.0.1:8766/templates/editorial-page/index.html

目录页可以切换 / 导入 / 导出主题。改 token 只改各模板自己的 `design-system/`，改完流式目录需重跑：
